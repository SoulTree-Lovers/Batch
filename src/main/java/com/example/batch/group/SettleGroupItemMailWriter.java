package com.example.batch.group;

import com.example.batch.domain.Customer;
import com.example.batch.domain.ServicePolicy;
import com.example.batch.domain.SettleGroup;
import com.example.batch.domain.repository.CustomerRepository;
import com.example.batch.support.EmailProvider;
...
public class SettleGroupItemMailWriter implements ItemWriter<List<SettleGroup>> {
    ...
    // 유료 API 총 사용 횟수, 총 요금, 세부사항에 대해서 메일 전송
    @Override
    public void write(Chunk<? extends List<SettleGroup>> chunk) throws Exception {
        for (List<SettleGroup> settleGroups : chunk) {
            ...
            final List<String> detailByService = settleGroups.stream().map(it ->
            ...
            final String body = """
                        안녕하세요. %s 고객님. 사용하신 유료 API 과금에 대해 안내드립니다.
                        총 %s 건을 사용하셨고, %s원의 비용이 발생했습니다.
