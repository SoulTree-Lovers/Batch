...
public class ApiOrderGenerateProcessor implements ItemProcessor<Boolean, ApiOrder> {
    ...
    private final List<Long> customerIds = LongStream.range(0, 21).boxed().toList(); // 0~20까지
    ...
    public ApiOrder process(Boolean item) throws Exception {
        ...
        final ApiOrder.State randomState = random.nextInt(5) % 5 == 1 ? ApiOrder.State.FAIL : ApiOrder.State.SUCCESS; // 20% 확률로 성공
